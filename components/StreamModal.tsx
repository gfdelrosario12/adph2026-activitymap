            ...
            <div className="bg-gradient-to-br from-slate-800 via-slate-900 to-slate-800 rounded-xl sm:rounded-2xl shadow-2xl max-w-5xl w-full max-h-[95vh] sm:max-h-[90vh] overflow-hidden border-2 border-green-500/30">
              ...
              <div className="relative bg-black aspect-video">
                {stream.embedUrl ? (
                  ...
                  // Show Thumbnail if no embed URL
                  <>
                    <Image
                    ...
                    <div className="absolute inset-0 flex items-center justify-center">
                      <div className="text-center">
                        <RadioIcon className="w-16 h-16 text-white/50 mx-auto mb-3" />
                        <p className="text-white text-lg font-semibold">Stream Preview</p>
                        <p className="text-slate-300 text-sm mt-1">Video player not available</p>
                      </div>
                    </div>
                  </>
                ) : (
                  // No thumbnail or embed URL
                  <div className="w-full h-full flex items-center justify-center bg-black">
                    <div className="text-center">
                      <RadioIcon className="w-24 h-24 text-slate-800 mx-auto mb-4" />
                      <p className="text-slate-600 text-lg">No video available</p>
                    </div>
                  </div>
                )}
                ...
                {/* Live Badge Overlay */}
                {stream.status === 'live' && (
                  <div className="absolute top-2 xs:top-3 sm:top-4 left-2 xs:left-3 sm:left-4 flex items-center gap-1.5 sm:gap-2 bg-red-600 px-2.5 xs:px-3 sm:px-4 py-1 sm:py-1.5 rounded-full text-[10px] xs:text-xs sm:text-sm font-bold z-10">
                ...
                {/* Quality Badge */}
                {stream.quality && (
                  <div className="absolute bottom-2 xs:bottom-3 sm:bottom-4 right-2 xs:right-3 sm:right-4 bg-black/60 backdrop-blur-sm px-2 xs:px-2.5 sm:px-3 py-0.5 xs:py-1 rounded-md text-[10px] xs:text-xs sm:text-sm font-semibold text-white z-10">
                    {stream.quality}
                  </div>
                )}
              </div>
